# Dokumentacja projektu

Ten katalog jest glownym zrodlem prawdy dla przygotowania i wdrozenia wersji Windows aplikacji Tyflocentrum.

## Start tutaj
- [README repo](../README.md)
- [Plan i backlog wdrozenia](windows-plan-and-backlog.md)
- [Roadmapa implementacji](implementation-roadmap.md)

## Produkt
- [Wymagania produktowe](product-requirements.md)
- [Macierz parytetu funkcji](feature-parity-matrix.md)
- [Otwarte pytania i decyzje do zamkniecia](open-questions.md)

## Analiza aplikacji bazowej
- [Analiza aplikacji iOS](source-analysis.md)
- [Integracje API i kontrakty](api-integrations.md)
- [Głosówki w Windows](voice-messages.md)

## Architektura i standardy
- [Architektura docelowa](architecture.md)
- [Standardy inzynierskie](engineering-standards.md)
- [Wymagania dostepnosci](accessibility-requirements.md)
- [Strategia testow](testing-strategy.md)
- [Setup deweloperski](developer-setup.md)

## Release
- [Pakowanie MSIX i Microsoft Store](release-msix-store.md)

## Decyzje architektoniczne
- [ADR 0001 - WinUI 3 + Windows App SDK + MSIX](adr/0001-winui3-windowsappsdk-msix.md)
- [ADR 0002 - Warstwowy uklad solution](adr/0002-layered-solution-structure.md)
